...
const PATH = "/signature-resize-to-50kb/";
const uc = SIGNATURE_KB_USECASES[50];

export const metadata = pageMetadata({
  title: "Signature Resize to 50KB – Free, Keeps Transparency",
  titleAbsolute: true,
  description:
    "Resize your signature to under 50 KB — for CSIR NET, UGC and portals that " +
    "allow a larger signature. Transparent background kept, in your browser. No upload.",
  path: PATH,
});
...
const faqItems = [
  ...
  uc.qualityFaq,
  {
    ...
    a: "Portals like UPSC, SSC and bank forms cap the signature file size (often 10–50 KB) and expect a clean background. A phone photo is usually far larger, so the upload is rejected.",
  },
  {
    ...
    <div className="container max-w-3xl py-10">
      <JsonLd
        schema={[
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Resize signature to 50KB", path: PATH },
          ]),
          softwareApplicationSchema({
            name: "Resize Signature to 50KB",
            description:
      ...
      <header className="mt-4 space-y-2">
        ...
        <p className="text-muted-foreground">
          Forms that want a 50 KB signature expect it to be clear and optimized. Scan or photograph your signature and this tool
          removes the paper, keeps the ink on a transparent background, and gets
          the file under 50 KB so the upload goes through. It all runs in your browser.
        </p>
      </header>

      <div className="mt-6">
        <SignatureWorkflowTool defaultTab="resize" defaultKb={50} autoCropDefault={true} />
      </div>
      ...
      <KbExamLinks kind="signature" kb={50} />
      ...
      <section className="mt-8">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/photo-resize-to-50kb/" className="rounded-md border border-hairline bg-card px-3 py-1.5 text-[13px] font-medium text-muted-foreground transition-colors hover:border-ink/30 hover:bg-accent/50 hover:text-foreground">
