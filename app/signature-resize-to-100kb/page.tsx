...
const PATH = "/signature-resize-to-100kb/";

export const metadata = pageMetadata({
  title: "Signature Resize to 100KB – Free, Keeps Transparency",
  titleAbsolute: true,
  description:
    "Resize your signature to under 100 KB online for free, with a transparent " +
...
const faqItems = [
  {
    ...
    a: "Portals like UPSC, SSC and bank forms cap the signature file size (often 10–100 KB) and expect a clean background. A phone photo is usually far larger, so the upload is rejected.",
  },
  {
    ...
    <div className="container max-w-3xl py-10">
      <JsonLd
        schema={[
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Resize signature to 100KB", path: PATH },
          ]),
          softwareApplicationSchema({
            name: "Resize Signature to 100KB",
            description:
              "Free tool to compress a signature to under 100 KB with a transparent background, in your browser.",
            url: PATH,
          }),
          howToSchema({
            name: "How to resize a signature to 100KB",
            description:
              "Get a transparent signature under 100 KB for online form uploads.",
            steps: [
              ...
              { name: "Download under 100 KB", text: "We trim and compress to under 100 KB as a transparent PNG, then you download it." },
            ],
          }),
        ]}
      />
      ...
      <header className="mt-4 space-y-2">
        <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
          Resize Signature to 100KB
        </h1>
        <p className="text-muted-foreground">
          Forms that want a 100 KB signature expect it to be clear and optimized. Scan or photograph your signature and this tool
          removes the paper, keeps the ink on a transparent background, and gets
          the file under 100 KB so the upload goes through. It all runs in your browser.
        </p>
      </header>

      <div className="mt-6">
        <SignatureWorkflowTool defaultTab="resize" defaultKb={100} autoCropDefault={true} />
      </div>
      ...
      <section className="mt-8">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/photo-resize-to-100kb/" className="rounded-md border border-hairline bg-card px-3 py-1.5 text-[13px] font-medium text-muted-foreground transition-colors hover:border-ink/30 hover:bg-accent/50 hover:text-foreground">
