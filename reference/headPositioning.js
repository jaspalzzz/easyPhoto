...
export const targetHeadMm = (spec) =>
 ...
 */
export function recommendedDigitalDpi(spec, baseDpi = 300) {
...
/**
 * @param {{chinY:number,crownY:number,eyeCenterY?:number,faceCenterX:number}} face
 * @param {object} spec  - a COUNTRY_SPECS entry
 * @param {object} [opts]
 ...
 * @returns {{crop,output,dpi,achieved,warnings}}
 */
export function computeCrop(face, spec, opts = {}) {
 ...
 * @param {CanvasImageSource} image - loaded <img>/ImageBitmap (the SOURCE)
 * @param {ReturnType<computeCrop>} result
 * @returns {HTMLCanvasElement}
 */
export function renderToCanvas(image, result) {
  ...
  ctx.drawImage(
    ...
    0, 0, output.width, output.height    // dest rect
  );
  return canvas;
}
